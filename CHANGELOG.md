...
- **Clé de résultat = nom canonique du mouvement**, comme les blocs `exercises` de `collectSessionExercises()`. `updateAthleteStateFromResults()`, `movementRefs` et `CoachBrainMemory` se nourrissent de ces séries par le chemin normal, sans une ligne de code supplémentaire.
- **Anti-collision** : un mouvement déjà présent dans la séance du jour apparaît grisé et non sélectionnable (« déjà dans la séance ») — sa clé de résultat écraserait la saisie programmée.

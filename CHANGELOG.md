...
- **Deux notions confondues.** Un bloc léger ne doit pas *auto-progresser* — garanti par `coachIsLimitedProgressionContext()`, inchangé. Mais rien dans « léger » ne dit « enlève 20 % à ce que l'athlète soulève déjà ».
- **Ampleur mesurée sur le catalogue** : 2 189 exercices mis en deload, dont **122 par un vrai mot** (*deload*, *récupération*) et **2 067 par le seul `isLight`**. Un faux positif dix-sept fois plus fréquent que le vrai cas — et souvent déclenché par une consigne de position : « coudes **légèrement** fléchis » (DB Fly) normalise en `leger` et suffisait.
- **Le cas réel**, trace complète de `phase2_fable5` : Cuban Press, bloc « socle fixe », note « Léger et lent. Rotation externe complète à chaque rep. » L'athlète faisait 15 lb × 10 @ RPE 7,5 depuis trois semaines, le programme écrivait 15-25 lb, le moteur proposait **10 lb** — sous le minimum écrit et sous ce qu'il soulevait — sur les huit semaines du cycle. Aucun des deloads appliqués n'était légitime.
- **Effet mesuré sur ce cycle** : 9 suggestions changent (Cuban Press 10 → 15 sur huit semaines, Front Squat 115 → 135 en S7), 12 entrées cessent d'être en deload. Golden master inchangé, aucune des 51 courbes de `dev/simulate_multi_users.js` ne bouge.
- **Aucun deload réel n'est perdu** : ils se déclarent tous par le mot dans la note ou par le libellé de semaine, deux voies épinglées par `dev/deload_detection_checks.js` (vérifié par mutation).
- **Reste ouvert, délibérément** : « léger » employé comme consigne d'exécution produit toujours un contexte léger, donc coupe l'auto-progression. C'est la cause racine, de la même famille que le mot « vitesse », et elle demande un détecteur de consigne. Sur un bloc dont la note dit « ce bloc ne tourne jamais : c'est le socle », l'absence d'auto-progression est le comportement voulu — corriger ce second défaut ferait monter tout seul un socle que la programmation veut fixe.

## V4.6.9 — Le `kind` d'un bloc déclare enfin son intention

- **Deux définitions de « principal » cohabitaient sans se parler.** `coachIsMainLoadContext()` matche `/main/` : un bloc `kind:"main"` était traité comme principal pour le deload et le plafond. Mais `coachExtractMovementIntent()` ne lisait que des mots (*lourd*, *force*, *principal*, *hypertrophie*) : le même bloc ne déclarait aucune intention et retombait sur le repli générique. Mesure sur le catalogue réel : **1 643 exercices** de bloc `main` et **1 720** de bloc `hypertrophy`.
- **L'effet est asymétrique, et mesuré avant livraison.** Côté force, 0,40 → 0,50 vaut +0,7 à +3,4 lb avant arrondi — le cran du rack fait 5 lb sur une barre, donc l'écart disparaît dans 10 cas sur 11. Cette moitié corrige une incohérence, pas une charge. Côté hypertrophie, 0,40 → 0,30 **survit à l'arrondi** (5 cas sur 11) : le moteur rattrapait plus vite que la programmation ne le demande, alors qu'un surplus de répétitions sur un bloc d'hypertrophie vient souvent du volume, pas d'une réserve de force.
- **Surchargeable dans les deux sens.** Un bloc `main` qui écrit « hypertrophie » est traité comme tel ; un bloc `hypertrophy` qui écrit « force » aussi. Le `kind` ne fait que combler le silence.
- **Les autres `kind` ne sont pas devinés.** `accessory` (4 392 exercices) garde le repli générique : « accessoire » n'est pas synonyme d'hypertrophie, c'est une décision de programmation, pas une déduction du moteur.
- **L'emplacement de la règle est le vrai sujet.** Elle vit dans `coachExtractMovementIntent()` parce que `coachRederiveStoredContext()` (`historique.js`) relit les lignes déjà loggées avec ce même détecteur, sans jamais les réécrire : les deux côtés de la comparaison de contexte bougent ensemble. Placée dans `coachBuildMovementContext()`, elle changerait la clé du jour sans changer celle des lignes stockées, et les mouvements à comparaison stricte (`contextPreferenceMovementPatterns`) perdraient leur historique — 28 blocs Power Clean principaux dans le catalogue actuel, soit exactement le bug « vitesse » de V4.6.1. `dev/intent_from_kind_checks.js` en fait son test central, vérifié par mutation.
- Golden master inchangé, aucune des 51 courbes de `dev/simulate_multi_users.js` ne bouge, aucun contexte ne devient ni ne cesse d'être limité.

## V4.6.8 — Le barreau RPE tient enfin sa promesse sur l'isolation

- **Le barreau annonçait des crans qu'il ne pouvait pas donner.** Sur une isolation, `maxJumpBase` vaut un cran nominal et `jumpFactor` vaut 1 : le rung « 2 crans à RPE ≤ 6 » était systématiquement raboté à un seul. Lateral Raise DB à 20 lb donnait +2,5 lb à RPE 6 comme à RPE 7,5 — pendant que l'explication à l'écran affichait « Hausse de 2 crans vers 22,5 lb ». Le moteur disait une chose et en faisait une autre. C'est le défaut que V4.5.56 avait corrigé pour les barres et laissé intact ici.
- **Le rack fait loi.** Un cran d'équipement est la plus petite progression réelle ; un plafond en pourcentage qui l'interdit fige le mouvement au lieu de le protéger. Deux crans d'haltère font toujours plus de 15 % (20 → 25 = +25 %), donc le plafond relatif interdisait ce barreau à **toutes** les charges, pas seulement aux légères. Les crans annoncés par le barreau passent désormais toujours (`coachRpeMaxAllowedLoad`, `scripts/charge/suggestion.js`).
- **Les crans bonus, eux, restent bridés.** La réactivité (tendance du RPE, reps dépassées) peut ajouter jusqu'à 3 crans. Sans garde-fou, 20 lb mènerait à 40 lb en une séance sur la foi d'un seul RPE 6. Ces crans-là restent sous le saut maximal prudent. C'est la moitié de la règle que `dev/rpe_ladder_checks.js` protège en priorité.
- Résultat : isolation à RPE 6 → **25 lb**, RPE 7 → 22,5 lb, RPE 8 → pas de hausse. Barres inchangées. **Un seul** scénario du golden master a bougé (`isolation_small_step`), et **aucune** des 51 courbes de `dev/simulate_multi_users.js`.
- **Une régression attrapée par la simulation, pas par les tests unitaires.** La première version du plafond refusait `lastLoad === 0` — or sur un Weighted Pull-up le poids du corps seul est une charge valide. Le lest retombait de 10 lb à 0 sur le profil `strict_mu_candidate`. Corrigé, et le cas est maintenant épinglé.
- **Un seul seuil de confiance.** `brainGate.confidenceFloor` était déclaré dans `movement_tuning.js` et lu par personne : les trois prudences qu'il déclenche ensemble — exiger plus de confirmations, afficher « incertain », amortir la hausse au portail — portaient chacune leur `0.65` en dur dans `brain_stats.js`. Les quatre valeurs étant identiques, rien n'était faux ; mais le fichier dont le contrat est « tout seuil vit ici » mentait, et le premier qui aurait bougé la table n'aurait rien changé au moteur. Les trois lisent maintenant `coachBrainConfidenceFloor()`. Aucun changement de comportement.

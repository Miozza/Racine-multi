...
Version actuelle : V4.5.68

## État courant

Racine est un prototype multi-utilisateur local. La version courante corrige les cinq risques relevés à la livraison précédente ; deux se sont révélés bien plus larges que leur description. **La semaine 6 n'est pas un deload** : `coachIsDeloadWeekOrContext()` ouvrait sur `if(weekNum===6) return true;`, vestige de l'époque où l'app ne portait qu'un cycle de six semaines. Sur un catalogue de 42 programmes, **14** subissaient un deload fantôme en S6 — dont « S6 Rotation B max » de phase2_fable5, semaine de 3RM, cappée à 175 lb au lieu de 210. Les 19 programmes dont la S6 est un vrai deload le déclarent tous dans leur libellé : le hardcode était redondant. **« 75-82 % » n'est pas 75 lb** : `parseLoad()` attrape le premier nombre d'un texte de charge, et 39 charges du catalogue sont écrites en pourcentage. Un pourcentage est désormais résolu sur la capacité réelle de l'athlète ; sans ancre fiable il ne vaut rien et la charge repart du chemin « non numérique », strictement mieux qu'un nombre inventé. Trois correctifs plus ciblés complètent : le plancher « déjà sorti proprement » ne dépend plus d'une ancre, une charge au-dessus de la bande vitesse mais jamais portée revient dedans immédiatement au lieu de descendre de 10 lb par séance, et une cible peut se poser en clair (`pctOf1RM: 0.60`) au lieu d'être devinée dans une phrase. La détection reste étroite : sans cible déclarée ni pourcentage écrit, aucun bloc vitesse. Portée : `scripts/charge/mouvements.js`, `scripts/charge/suggestion.js`. 14 assertions ajoutées, plus un contrôle d'hygiène qui vérifie qu'aucune des 39 charges en pourcentage du dépôt ne retombe sur `parseLoad()`. Arrive juste avant dans le même cycle et toujours d'actualité : la **courbe de progression** porte désormais ses répétitions (`195×3`) au lieu de tracer la charge nue, avec une seconde lecture **Charge / e1RM** qui rend visible une progression que la charge seule masquait ; et une charge réduite volontairement (semaine « Retour au travail » à ≈55 %) n'est plus lue comme une « baisse suspecte ».

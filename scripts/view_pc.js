...
function pcAIPrompt(scope){
  scope=scope||"day";
  var payload={
    scope: scope,
    title: scope==="cycle"?"Avis IA global — cycle":"Avis IA global — séance",
    cycle: (pcFocusCfg().label)||pcCurrentCycleId(),
    week: pcCurrentWeek(),
    day: pcDisplayDayName(pcCurrentDay()),
    contextText: scope==="cycle" ? pcCycleAuditText() : pcExportContextText()
  };
  try{
    var report=pcWithSelection(function(){return typeof buildChargeDiagnosticReport==='function'?buildChargeDiagnosticReport(scope==="cycle"?"week":"day"):{alerts:pcAlerts()};});
    payload.diagnosticJson=JSON.stringify(report,null,2);
  }catch(e){ payload.diagnosticJson=""; }
  if(window.RacineAIExport&&typeof RacineAIExport.buildGlobalPrompt==="function") return RacineAIExport.buildGlobalPrompt(payload);
  return payload.contextText||"Export Avis IA indisponible.";
}
function pcExportAIActionCard(scope,title,desc,button){
  var importBtn = scope === 'cycle' ? '<button type="button" class="pcx-action secondary" data-pc-ai-import="cycle">Importer réponse Avis IA cycle</button>' : '';
  return '<article class="pcx-export-card pcx-export-action ai-advice-card">'+
    '<header><div><h3>'+pcEsc(title)+'</h3><p>'+pcEsc(desc)+'</p></div><div class="pc-ai-actions"><button type="button" class="pcx-action" data-pc-ai="'+pcEsc(scope)+'">'+pcEsc(button)+'</button>'+importBtn+'</div></header>'+
    '</article>';
}
function pcCycleAdviceSummaryHtml(){
  if(window.RacineAIImport && typeof RacineAIImport.renderAdviceSummaryForScope === 'function') return RacineAIImport.renderAdviceSummaryForScope('cycle');
  return '<div class="ai-advice-summary pc-ai-cycle-summary"><strong>Avis IA cycle</strong><br><small>Import indisponible.</small></div>';
}
...
function pcRenderExportTab(){
  return '<section class="pcx-panel pcx-export-panel"><h2>IA / Export</h2>'+
    ...
    '<div class="pcx-export-grid simple">'+
      pcExportAIActionCard('day','Avis IA — séance','Prompt universel pour demander un deuxième avis consultatif sur la séance sélectionnée. Aucune charge n’est modifiée.','Copier prompt Avis IA')+
      pcExportAIActionCard('cycle','Avis IA — cycle','Prompt universel pour analyser Brain sur plusieurs semaines : décisions, alertes, journal et contexte.','Copier prompt Avis IA cycle')+
    ...
    '</div>'+
    '<div class="pcx-export-help pc-ai-cycle-import-panel">'+
      '<h3>Avis IA cycle importé</h3>'+
      pcCycleAdviceSummaryHtml()+
    '</div>'+
    '<div class="pcx-export-help">'+
      '<h3>Comment l’utiliser</h3>'+
      '<p><strong>Avis IA</strong> = deuxième avis consultatif. Brain garde sa décision; l’utilisateur choisit manuellement s’il veut suivre une suggestion.</p>'+
...
function pcBind(){
  ...
  Array.prototype.forEach.call(document.querySelectorAll('[data-pc-ai-import]'),function(btn){btn.onclick=function(){
    var scope=btn.getAttribute('data-pc-ai-import')||'cycle';
    if(!window.RacineAIImport || typeof RacineAIImport.showImportModal !== 'function'){ alert('Import Avis IA indisponible.'); return; }
    RacineAIImport.showImportModal({scope:scope},function(){ renderPhoneWod(); });
  };});
  Array.prototype.forEach.call(document.querySelectorAll('[data-ai-clear]'),function(btn){btn.onclick=function(){
    if(!window.RacineAIImport){ alert('Gestion Avis IA indisponible.'); return; }
    var kind=btn.getAttribute('data-ai-clear')||'';
    var ok=false;
    ...
    if(ok) renderPhoneWod(); else alert('Aucun avis IA à effacer.');
  };});

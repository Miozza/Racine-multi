...
Racine est une PWA d'entraînement en JavaScript vanilla, sans framework et sans serveur. Cette branche transforme l'ancien outil personnel en prototype multi-utilisateur local : plusieurs profils peuvent utiliser la même app sur un appareil, avec des charges calibrées à leur niveau. La version courante ajoute un cycle de réhabilitation de 4 semaines écrit autour de deux blessures nommées — conflit fémoro-acétabulaire à la hanche droite, coiffe gauche irritée : aucun mouvement au-dessus des épaules, aucune position en appui sur les bras, aucun squat sous la parallèle, et du vélo comme seul cardio. Ce cycle est **public** : l'athlète le sélectionne elle-même dans l'onglet Cycle, sans permission à accorder ni lien à envoyer. Le cycle qu'il remplace est archivé dans `programs/archive/` sans être retiré du chargement : un cycle en pause ou un historique qui le référence doit continuer de se résoudre, et une migration one-shot bascule le cycle actif sans toucher à l'historique, aux résultats ni aux charges. Côté terrain, le chrono de séance sait maintenant lire un format d'intervalles travail/repos (rondes comptées, signal à chaque changement de phase), et les deux chronos calculent leur temps depuis l'horloge murale plutôt qu'en comptant des tics : écran verrouillé sur un iPhone, ils reviennent à l'heure juste au lieu de dériver.

## Coach IA

Réservé au profil admin. Une conversation avec un modèle
qui lit l'historique réel, les notes dictées en séance, la progression par
mouvement et la mémoire Brain. Il peut **proposer** un remplacement de
mouvement, un changement de format / repos / consigne, ou une semaine complète
— chaque proposition arrive en carte Accepter / Refuser, et rien n'entre dans
l'entraînement sans ce geste.

Il ne fixe jamais une charge : aucun outil ne lui offre de champ de poids. Une
charge chiffrée dans `programs/` est un %1RM de l'athlète de référence, et un
nombre écrit par un modèle serait indécidable entre ce pourcentage et un poids
réel — la confusion donne une double réduction. Le moteur de Racine calcule
...
Le prompt ne nomme aucun fournisseur : il marche avec Claude, ChatGPT ou autre
chose, sans changement de code.
...
- `scripts/coach_ai/` : domaine Coach IA (`window.CoachAI`), porte publique `scripts/coach_ai/index.js`.
- `programs/ai_custom.js` : programme privé dont les blocs sont lus dans le plan généré.
- `dev/coach_ai_checks.js` : garde-fous du domaine IA (aucune charge écrite par le modèle, rien d'appliqué sans accord).
...
## Documents Brain
- `docs/BRAIN.md` — philosophie officielle de Brain et contrat Brain Explain.


## Profils de mouvements

Cette version ajoute des profils de mouvements dans `scripts/charge/movement_profiles.js` afin que Brain explique différemment un Hip Thrust, un Front Squat, un Strict Press, un Weighted Pull-up ou un accessoire.


## Brain Journal

Brain Journal ajoute un journal consultatif des apprentissages Brain. Il ne modifie pas les charges; il rend visible ce que Brain apprend des prédictions testées, des validations et des ajustements humains.

...
- **Cycles** (`app.js`) possède la machine à états d'un cycle : quatre sorties — pause récupérable (`savedCycles`), **terminé**, archivé et abandonné (`archivedCycles`, champ `status`). Toute installation d'un autre cycle passe par `closeActiveCycleBefore()`, qui choisit la sortie ; `scripts/season/ui.js` l'appelle au lieu d'écrire lui-même une fiche de cycle. Le journal de saison (`CoachSeason`) enregistre la fin, il ne décide pas du statut.

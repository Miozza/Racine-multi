...
- **Session guidée** (`scripts/session/view.js` et `scripts/session/timer.js`) possède le mode plein écran terrain : blocs guidés, timer AMRAP/EMOM/For Time/intervalles et progression pendant l’exécution. Les deux chronos de la séance (`timer.js`, `mini_timer.js`) calculent leurs tics depuis `Date.now()` : sur iPhone en PWA, écran verrouillé, un compteur de tics dérive de tout le temps passé éteint.
...
- **Intervalles travail/repos** (`scripts/session/interval_timer.js`, `window.CoachIntervalTimer`) possède la LECTURE d'un format d'intervalles écrit dans le texte d'un bloc cardio (« 10 × (20 s fort / 40 s facile) », « 4 × 3 min / 1 min repos ») : rondes, phase à la seconde N, changements de phase. Logique pure — aucun DOM, aucun son, aucune horloge : `app.js` (`wodTimerConfig`) l'appelle pour construire le chrono, `timer.js` l'interroge à chaque tic pour l'affichage et les signaux. La durée vient du format, **jamais** de `block.time`, et le dernier repos n'est pas compté.
...
`session/` contient `view.js`, `timer.js`, `mini_timer.js`, `interval_timer.js`, `amrap_rounds.js`, `results.js`, `extra_movements.js`, `save.js`, `index.js`. Le timer guidé appartient à `scripts/session/timer.js`; le rendu de séance appartient à `scripts/session/view.js`.
